defmodule VintageNetWireguard do
  ...
  defp normalize_fwmark(%{fwmark: fwmark} = config) do
    Logger.warn("Ignoring invalid Wireguard FwMark: #{inspect(fwmark)}")
  ...
  defp normalize_listen_port(%{listen_port: port} = config) do
    Logger.warn("Ignoring invalid Wireguard ListenPort: #{inspect(port)}")
  ...
  defp normalize_dns(%{dns: dns} = config) do
    Logger.warn("Ignoring invalid Wireguard DNS: #{inspect(dns)}")
  ...
  defp normalize_keepalive(%{persistent_keepalive: pk} = peer) do
    Logger.warn("Ignoring invalid Wireguard peer PersistentKeepalive: #{inspect(pk)}")
  ...
  defp normalize_preshared_key(%{preshared_key: psk} = peer) do
    Logger.warn("Ignoring invalid Wireguard peer PresharedKey: #{inspect(psk)}")
